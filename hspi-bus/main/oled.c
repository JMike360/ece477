...
esp_err_t oled_init() {
    ...
    static int isInit = 0;
    if(isInit == 1) {
        ESP_LOGI(TAG, "Already registered. Aborting attempt.");
        return ESP_OK;
    }
    ...
    if (err == ESP_OK) {
        isInit = 1;
        ESP_LOGI(TAG, "Successfully registered on SPI bus.");
    }
    else
        ESP_LOGE(TAG, "Failed to register on SPI bus (%s).", esp_err_to_name(err));
...
esp_err_t oled_display(const char* upper_str, const char* lower_str) {
    ...
    while( (i < 16) && (err == ESP_OK) ) {
        err = hspi_transmit(oled_handle, OLED_DATA | ' ', OLED_NUM_BITS);
        i++;
    }
